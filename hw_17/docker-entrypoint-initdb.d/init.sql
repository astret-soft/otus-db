...
-- ЗАМЕЧАНИЯ: скрипт необязателен, можно было импортировать CSV через pgAdmin или создавать временную таблицу с минимальными ограничениями на поля и далее
...
-- * Создаем many-to-many между адресом и клиентом (если все же one-to-many, то person_id нужно добавить ограничение на уникальность: UNIQUE):
...
-- Создем индексы над сущностями, где это надо FTS для поиска по адрессу или ФИО клиента...
